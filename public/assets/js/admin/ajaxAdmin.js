...
function handleCorrectFilm(response) {
  const filmHTML = generateRecapHTML(response.filmChecked, response);
  $("#container-recap").html(filmHTML);
  applyFilmBackground(response.filmChecked.poster_path, nbEssais);
  showCongratulationMessage(response.filmChecked.original_title);
}
...
// Génère le HTML pour la div de récapitulatif
function generateRecapHTML(filmChecked, response) {
  let html = `
    <div class='card essai' id='recap-film'>
      <h4>${filmChecked.original_title}</h4>
      <div class='row container-details'>${generateGenresHTML(filmChecked.genres)}</div>
      <div class='row container-details'>${generatePaysHTML(filmChecked.pays)}</div>
      <div class='row container-details'>${generateProductionsHTML(filmChecked.productions)}</div>
      <div class='row container-details'>
        <div class='row details'>${filmChecked.release_date}</div>
      </div>
      <div class='row container-details'>
        <div class='row row-nb-acteur-real'>
          <p>Acteurs ${filmChecked.acteurs.length}/${filmChecked.acteurs.length}</p>
          <p>Réal(s) ${filmChecked.realisateurs.length}/${filmChecked.realisateurs.length}</p>
        </div>
        <div class="row container-personne-row">
          <div class='row scrollable-row acteur-row'>${generateActeursHTML(filmChecked.acteurs)}</div>
          <div class='row scrollable-row real-row' id='find-real-row'>${generateRealisateursHTML(filmChecked.realisateurs)}</div>
        </div>
      </div>
    </div>`;
  return html;
...
function applyFilmBackground(posterPath, nbEssais) {
  const cssProperties = {
    "background-image": `linear-gradient(rgb(40, 31, 74) 2%, rgba(40, 31, 74, 0.7) 50%, rgb(40, 31, 74, 0.7)), url('https://image.tmdb.org/t/p/w500${posterPath}')`,
    "background-size": "cover",
    "background-position": "center"
  };
  $("#recap-film").css(cssProperties);
}
...
function generateIncorrectFilmHTML(response) {
  let html = `
    <div class='card essai' id='essai-${nbEssais}'>
      ...
      <div class='row container-details'>${generateIncorrectProductionsHTML(response)}</div>
      <div class='row container-details'>
...
// Génère le HTML pour les productions incorrectes
...
// Génère le HTML pour les productions du film vérifié
